...
3. **Brief intake — the exhaustive grill-me interview.** The Brief is the highest-leverage artifact in the system: everything downstream inherits its errors, and a vague Brief is the usual reason a concept trio comes back generic. Run it **in full on every campaign — no exceptions** (operator ruling 2026-09-06). Full discipline + the topic bank: [`brief.md`](../../../docs/specs/brief.md) §"Interview discipline (grill-me pattern) — v4".
   - **There is NO time target.** The old 4–8 minute target is removed — it was a budget capping depth. The interview ends on a **coverage bar**: every topic in the bank is `answered` · `assumed` (assumption written down) · `homework` (owner + stated default) · `inherited` (settled at the tenant layer, with a pointer) · or `n/a` (with the reason). **≤2 questions per turn still holds** — that's pacing, not depth; fifteen questions in one message returns fifteen shallow answers.
   - **Step 0 — SWEEP BEFORE YOU ASK.** Assemble "What we already know" from what the system already holds: prior campaigns for this tenant (`campaigns/*/campaign.yaml` KPI blocks · any `analysis/` folder · campaign reports), playbook §0 + §0a, `tenant-brand/<tenant>-audience-truths.md`, `-market.md`, the research library, `tenant/library/`. Then **open with the read, not a question**: *"Here's what I already know about this audience and what your last campaigns in this channel actually did — correct me where I'm wrong."* Grilling someone on facts the system could look up is what turns an interview into an interrogation, and it gets worse answers than reacting to something concrete.
   - **What earns a question: grill on EVIDENCE, recommend on DECISIONS.** If AI can look it up, research it, or reasonably assume it, it is not a question — it's a stated read the operator confirms or corrects. Only what lives solely in the marketer's head gets asked. Same contract as everywhere else: don't push operational labour up.
   - **Never re-ask the tenant layer.** Brand Context and playbook §0/§0a are FIXED INPUTS — confirm in one line, never re-interview. If an answer CHANGES a tenant-level fact it graduates UP to the playbook; it does not get buried in a campaign Brief.
   - **Per topic**: state your read → ask 1–2 probing questions → recommend (use `AskUserQuestion` where there's an option set) → capture with attribution → record the status in the ledger → move on. Cover all eight territories: **A** objective & outcome · **B** what we already know · **C** competitors & category · **D** audience & research · **E** the offer · **F** proposition & message · **G** constraints · **H** risk.
   - **KPI baseline check (A3/A4) — do not skip.** Every KPI records the **baseline it was derived from**; flag a target that implies an unexplained multiple of it. Not theoretical: Soundtrak's brand-bank "45,000+ / 16,500+" are *inherited* audience numbers, not owned reach (the real figures are ~50 Substack subscribers and ~3k LinkedIn followers) — a content KPI set against those would have been unreachable by an order of magnitude and nothing in the old Brief would have caught it. "First time in this channel, no baseline" is a legitimate answer; silence is not.
   - **Pre-mortem (H1)**, near the end, one question: *"It's the deadline and this flopped — what happened?"* People name risks in that frame they won't volunteer in any other.
   - **Differentiation check (C5)**: if our intended claim is what everyone in the category already says, surface it now rather than discovering it at concept stage.
   - **HOMEWORK — when the operator has to go and collect something.** Don't accept a guess. Try AI first (research, a reachable public source, an existing export) per the orchestrates-not-delegates contract; only then set homework. Each item records **what** is needed + **where to find it**, **why it's load-bearing**, and **the default we'll assume if it never arrives** — so a decline never blocks. Then classify it:
     - **load-bearing → BLOCKS Brief approval.** Being wrong here would invalidate the work. Say so plainly and park the campaign in that state, with the item as a real dashboard To Do.
     - **useful → PROCEEDS under the stated assumption**, visible on the Brief and in the ledger so the operator can see what the campaign rests on.
   - **Write the transcript.** The Brief is a 1–2 page distillation; the interview produces far more, and that surplus is what the CD and Producer need. Capture the full Q&A in `campaigns/<slug>/research/intake-notes.md` and link it from the Brief. Nothing gets thrown away.
   - **Dependency pass** after the topics: surface the tensions (budget vs effort · timeline vs KPI · tech setup vs asset list · **KPI vs baseline**) and resolve before drafting.
   - **Fast-path is a PACING override only** (changed 2026-09-06): "fast-path"/"batch" lets the operator answer in bulk instead of turn-by-turn. It does **NOT** reduce coverage — every topic still lands in the ledger with a status.
4. **Author the Brief** (1-2 pages; pull aggressively from inputs; flag `TBD`s in the approval surface). It MUST carry **What we already know** (the evidence base — prior campaigns and their real numbers · channel benchmark · competitors + the differentiation check · audience research we already hold · gaps) as section 2, and the **interview coverage ledger** as a collapsed block in the Approval record. `brief_lint.py` holds any brief carrying a ledger to that contract and FAILS a ledger row with no status — a blank row reads as coverage while proving nothing.
